...
class Scheduler (threading.Thread):
  ...
  def schedule_task(self, channel, message, sender, trigger_time=None, trigger_delay=None, send_time=None, source_channel=None):
    if self.parent_bot.DEBUG:
      self.parent_bot.debug_print('registering task: ' + channel + ' ' + message + ' ' + sender)
    send_time = datetime.now(timezone.utc)
    if trigger_delay is not None:
      td = timedelta(seconds=parse(trigger_delay))
      trigger_delay = send_time + td
    t = self.Task(channel, message, sender, trigger_time, trigger_delay, send_time)
