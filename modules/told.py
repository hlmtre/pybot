...
class Told(BaseModule):
    ...
    def get_told_status(self, target):
        ...
        return random.choice(
            exclamation) + " %s\'s told status: [X] " % target + random.choice(status)

    def handle(self, event):
        # Splits the user out that will be "told"
        _z = event.msg.split(None, 1)
        try:
            # If there is a user he will get a "told" status
            self.say(event.channel, self.get_told_status(_z[1]))
        except IndexError:
            # If the person doing the told does not list anyone, they will be
            # "told" themselves
