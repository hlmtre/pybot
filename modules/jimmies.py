...
class Jimmies(BaseModule):
  ...
  def handle(self, event):
    ...
    jimmies_status = self.get_jimmies_status() # Gets the jimmies status from the list above
    try:
      self.say(event.channel, "Jimmies status for " + _z[1]+ ": " + jimmies_status) # Prints the jimmies status to the proper channel if a user is specified properly
    except IndexError:
