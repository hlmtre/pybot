...
class recap(BaseModule):
    ...
    def scramble_nick(self, nick):
        ...
        try:
            vowels = 'aeiou'
            nick_vowels = []
            # grab the nick from between <> and conver to a list to make
            # changes
            nick_letters = list(nick[1:-1])
            # create a list of tuples. each tuple is (index of a vowel in nick,
            # the vowel at that index)
            ...
            # keep doing the previous line until we get something different
            # from what we're replacing
            while repl == sel[1].lower():
                repl = random.choice(vowels)
            # if the chosen letter to be replaced is upper case, make sure the
            # replacement is too
            if nick_letters[sel[0]].isupper():
                nick_letters[sel[0]] = repl.upper()
            else:
                nick_letters[sel[0]] = repl
            # take that list of individual characters and slam it all back
            # together into a string surrounded by <>
            nick = '<' + ''.join(nick_letters) + '>'
            # take the old nick out of the submitted line and replace it with
    ...
    def check_rate(self, channel):
        ...
        try:
            if self.get_timediff(channel) <= 0:
                self.bot.mem_store['recap'][channel] = int(
                    time.time()) + self.RATE_LIMIT
                return True
            else:
                return False
        except KeyError:
