...
class Replace(BaseModule):
    ...
    def add_buffer(self, event=None, debug=False):
        ...
        if not event.channel:
            ...
            try:
                for chan in list(self.bot.mem_store['replace'].keys()):
                    if len(self.bot.mem_store['replace']
                ...
                self.bot.logger.write(
                    Logger.WARNING,
                    line="couldn't find channel in modules.replace")
                self.bot.logger.write(
                    Logger.WARNING,
                    line="Replace add_buffer() error when no event channel")
        ...
        # END if not event.channel:
            ...
            try:
                ...
                line = self.format_line(event)
                if line:
                    self.bot.mem_store['replace'][event.channel].insert(
                        0, line)
            except IndexError:
                self.bot.logger.write(
                    Logger.WARNING,
                    line="Replace add_buffer() error. Couldn't access the list index.")
    ...
    def handle(self, event):
        ...
        if event.msg.startswith(".r "):
            # split the msg with '.r ' stripped off beginning and divide into a
            # search string and replace string
            string_token = event.msg[3:].split('|', 1)
            find_msg = string_token[0].rstrip()
            try:
                # if there's nothing after the pipe, then this resolves to ''
                # which is fine
            ...
            # this actually works to our advantage so we dont have to do
            # additional calls to find out who sent what
        ...
        if event.msg.startswith("s/"):
            self.bot.logger.write(
                Logger.WARNING,
                line="i am a log entry from within s/ in modules.replace")
            # alternative notation: s/<substring to replace>/<substring to
            # replace with>
